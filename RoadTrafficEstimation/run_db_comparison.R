...
pipeline_message(text = "Standard vs dB-aware emission comparison",
...
# Helper: align sparse matrix columns to what a model expects
...
cat("\n")
cat("╔══════════════════════════════════════════════════════════════════════╗\n")
cat("║         COMPARISON: STANDARD vs dB-AWARE XGBoost (period D test)   ║\n")
cat("╠══════════════════════════════════════════════════════════════════════╣\n")
cat(sprintf("║  %-32s  %12s  %12s  ║\n", "Metric", "STANDARD", "dB-AWARE"))
cat("╠══════════════════════════════════════════════════════════════════════╣\n")
cat(sprintf("║  %-32s  %12d  %12d  ║\n", "N test samples", s_std$n, s_db$n))
cat("╠══════════════════════════════════════════════════════════════════════╣\n")
cat(sprintf("║  %-32s  %+11.2f  %+12.2f  ║\n", "Bias (dB)", s_std$bias, s_db$bias))
cat(sprintf("║  %-32s  %12.2f  %12.2f  ║\n", "MAE (dB)", s_std$mae, s_db$mae))
cat(sprintf("║  %-32s  %12.2f  %12.2f  ║\n", "RMSE (dB)", s_std$rmse, s_db$rmse))
cat(sprintf("║  %-32s  %12.2f  %12.2f  ║\n", "MedAE (dB)", s_std$medae, s_db$medae))
cat(sprintf("║  %-32s  %12.2f  %12.2f  ║\n", "P90 |Error| (dB)", s_std$p90ae, s_db$p90ae))
cat("╠══════════════════════════════════════════════════════════════════════╣\n")
cat(sprintf("║  %-32s  %+11.2f  %+12.2f  ║\n", "Bias — Flow contrib (dB)", s_std$bias_flow, s_db$bias_flow))
cat(sprintf("║  %-32s  %+11.2f  %+12.2f  ║\n", "Bias — Truck% contrib (dB)", s_std$bias_truck, s_db$bias_truck))
cat(sprintf("║  %-32s  %+11.2f  %+12.2f  ║\n", "Bias — Speed contrib (dB)", s_std$bias_speed, s_db$bias_speed))
cat("╠══════════════════════════════════════════════════════════════════════╣\n")
cat(sprintf("║  %-32s  %12.2f  %12.2f  ║\n", "MAE — Flow contrib (dB)", s_std$mae_flow, s_db$mae_flow))
cat(sprintf("║  %-32s  %12.2f  %12.2f  ║\n", "MAE — Truck% contrib (dB)", s_std$mae_truck, s_db$mae_truck))
cat(sprintf("║  %-32s  %12.2f  %12.2f  ║\n", "MAE — Speed contrib (dB)", s_std$mae_speed, s_db$mae_speed))
cat("╠══════════════════════════════════════════════════════════════════════╣\n")
cat(sprintf("║  %-32s  %+11.2f  %+12.2f  ║\n", "Bias with OSM speed (dB)", s_std$bias_osm, s_db$bias_osm))
cat(sprintf("║  %-32s  %12.2f  %12.2f  ║\n", "MAE with OSM speed (dB)", s_std$mae_osm, s_db$mae_osm))
cat(sprintf("║  %-32s  %12.2f  %12.2f  ║\n", "RMSE with OSM speed (dB)", s_std$rmse_osm, s_db$rmse_osm))
cat("╚══════════════════════════════════════════════════════════════════════╝\n")

# Improvement summary
cat("\n")
cat(sprintf("  Δ Bias:  %+.2f → %+.2f dB  (Δ = %+.2f)\n",
            s_std$bias, s_db$bias, s_db$bias - s_std$bias))
cat(sprintf("  Δ MAE:   %.2f → %.2f dB  (Δ = %+.2f, %+.1f%%)\n",
            s_std$mae, s_db$mae, s_db$mae - s_std$mae,
            (s_db$mae - s_std$mae) / s_std$mae * 100))
cat(sprintf("  Δ RMSE:  %.2f → %.2f dB  (Δ = %+.2f, %+.1f%%)\n",
            s_std$rmse, s_db$rmse, s_db$rmse - s_std$rmse,
            (s_db$rmse - s_std$rmse) / s_std$rmse * 100))
cat(sprintf("  Δ Speed bias: %+.2f → %+.2f dB  (Δ = %+.2f)\n",
            s_std$bias_speed, s_db$bias_speed,
            s_db$bias_speed - s_std$bias_speed))
cat("\n")
...
hw_db <- as.data.table(emission_db)[, .(
  ...
  speed_bias_db = mean(contrib_speed)
), by = highway]

hw_comp <- merge(hw_std, hw_db, by = "highway", all.x = TRUE)
hw_comp <- hw_comp[order(-n)]
hw_comp$delta_mae <- hw_comp$mae_db - hw_comp$mae_std
hw_comp$delta_speed_bias <- hw_comp$speed_bias_db - hw_comp$speed_bias_std

top_hw <- hw_comp[1:min(10, nrow(hw_comp))]

cat("\nPer-highway MAE comparison:\n")
for (i in 1:nrow(top_hw)) {
  h <- top_hw[i]
  cat(sprintf("  %-15s (n=%5d): MAE %.2f→%.2f dB (Δ%+.2f) | Speed bias %+.2f→%+.2f dB\n",
              h$highway, h$n, h$mae_std, h$mae_db, h$delta_mae,
              h$speed_bias_std, h$speed_bias_db))
}
...
cat(sprintf("  %-10s %6s  %8s %8s %8s  %8s %8s %8s\n",
            "Speed", "N", "Bias_std", "MAE_std", "RMSE_std",
            "Bias_dB", "MAE_dB", "RMSE_dB"))
for (i in 1:nrow(sb_comp)) {
  s <- sb_comp[i]
  cat(sprintf("  %-10s %6d  %+7.2f  %7.2f  %7.2f   %+7.2f  %7.2f  %7.2f\n",
              as.character(s$speed_bin), s$n,
              s$bias_std, s$mae_std, s$rmse_std,
              s$bias_db, s$mae_db, s$rmse_db))
}
...
cat(sprintf("\nCorrelation r(dB_error, actual_speed):\n"))
...
# ==============================================================================
# 8. Generate comparison PDF
# ==============================================================================

pipeline_message(text = "Generating comparison PDF",
                 level = 1, progress = "start", process = "plot")

pdf_path <- file.path(CONFIG$FIGS_DIR, "08_standard_vs_db_aware_comparison.pdf")
...
# --- Page 1: Side-by-side error distributions ---
par(mfrow = c(2, 2), mar = c(5, 5, 4, 2))

# 1a: Error histograms
hist(emission_std$db_error, breaks = 80, col = rgb(0.2, 0.4, 0.8, 0.5),
     main = "dB Error Distribution — Standard",
     xlab = "Prediction error (dB)", ylab = "Count", xlim = c(-20, 20))
abline(v = mean(emission_std$db_error), col = "red", lwd = 2, lty = 2)
abline(v = 0, col = "black", lwd = 1)
legend("topright", legend = sprintf("Bias=%+.2f dB\nMAE=%.2f dB\nRMSE=%.2f dB",
       s_std$bias, s_std$mae, s_std$rmse), bty = "n", cex = 1.1)

hist(emission_db$db_error, breaks = 80, col = rgb(0.8, 0.3, 0.2, 0.5),
     main = "dB Error Distribution — dB-aware",
     xlab = "Prediction error (dB)", ylab = "Count", xlim = c(-20, 20))
abline(v = mean(emission_db$db_error), col = "red", lwd = 2, lty = 2)
abline(v = 0, col = "black", lwd = 1)
legend("topright", legend = sprintf("Bias=%+.2f dB\nMAE=%.2f dB\nRMSE=%.2f dB",
       s_db$bias, s_db$mae, s_db$rmse), bty = "n", cex = 1.1)

# 1b: Error vs speed scatter
...
# --- Page 2: Bias decomposition comparison ---
par(mfrow = c(2, 2), mar = c(5, 5, 4, 2))

# 2a: Bias decomposition bar chart
...
rownames(bias_data) <- c("Standard", "dB-aware")

barplot(bias_data, beside = TRUE,
        col = c(rgb(0.2, 0.4, 0.8, 0.7), rgb(0.8, 0.3, 0.2, 0.7)),
        main = "Bias Decomposition by Variable",
        ylab = "Bias (dB)", ylim = c(min(bias_data) * 1.3, max(bias_data) * 1.3))
abline(h = 0, col = "gray50")
legend("topleft", legend = c("Standard", "dB-aware"),
       fill = c(rgb(0.2, 0.4, 0.8, 0.7), rgb(0.8, 0.3, 0.2, 0.7)))

# 2b: MAE decomposition bar chart
...
rownames(mae_data) <- c("Standard", "dB-aware")

barplot(mae_data, beside = TRUE,
        col = c(rgb(0.2, 0.4, 0.8, 0.7), rgb(0.8, 0.3, 0.2, 0.7)),
...
# 2c: Speed-binned MAE comparison
if (nrow(sb_comp) > 0) {
  sb_mat <- matrix(c(sb_comp$mae_std, sb_comp$mae_db), nrow = 2, byrow = TRUE)
  colnames(sb_mat) <- as.character(sb_comp$speed_bin)
  barplot(sb_mat, beside = TRUE,
          col = c(rgb(0.2, 0.4, 0.8, 0.7), rgb(0.8, 0.3, 0.2, 0.7)),
...
# 2d: Speed-binned bias comparison
if (nrow(sb_comp) > 0) {
  sb_bias_mat <- matrix(c(sb_comp$bias_std, sb_comp$bias_db), nrow = 2, byrow = TRUE)
  colnames(sb_bias_mat) <- as.character(sb_comp$speed_bin)
  barplot(sb_bias_mat, beside = TRUE,
          col = c(rgb(0.2, 0.4, 0.8, 0.7), rgb(0.8, 0.3, 0.2, 0.7)),
...
if (nrow(top_hw) > 0) {
  hw_mat <- matrix(c(top_hw$mae_std, top_hw$mae_db), nrow = 2, byrow = TRUE)
  colnames(hw_mat) <- paste0(top_hw$highway, "\n(n=", top_hw$n, ")")
  barplot(hw_mat, beside = TRUE,
          col = c(rgb(0.2, 0.4, 0.8, 0.7), rgb(0.8, 0.3, 0.2, 0.7)),
          main = "MAE by Highway Type — Standard vs dB-aware",
          ylab = "MAE (dB)", las = 2, cex.names = 0.8)
  legend("topright", legend = c("Standard", "dB-aware"),
         fill = c(rgb(0.2, 0.4, 0.8, 0.7), rgb(0.8, 0.3, 0.2, 0.7)))
}

# --- Page 4: QQ plot of dB errors ---
par(mfrow = c(1, 2), mar = c(5, 5, 4, 2))
qqnorm(emission_std$db_error, main = "QQ Plot — Standard dB Errors",
       col = rgb(0.2, 0.4, 0.8, 0.3), pch = 20, cex = 0.5)
qqline(emission_std$db_error, col = "red", lwd = 2)

qqnorm(emission_db$db_error, main = "QQ Plot — dB-aware dB Errors",
       col = rgb(0.8, 0.3, 0.2, 0.3), pch = 20, cex = 0.5)
qqline(emission_db$db_error, col = "red", lwd = 2)

# --- Page 5: Pred vs Actual dB ---
par(mfrow = c(1, 2), mar = c(5, 5, 4, 2))
lim <- range(c(emission_std$actual_db, emission_std$pred_db,
               emission_db$actual_db, emission_db$pred_db), na.rm = TRUE)

plot(emission_std$actual_db, emission_std$pred_db,
     pch = ".", col = rgb(0, 0, 1, 0.1), cex = 2,
     main = sprintf("Pred vs Actual dB — Standard\nR²=%.3f",
                     1 - sum((emission_std$pred_db - emission_std$actual_db)^2) /
...
plot(emission_db$actual_db, emission_db$pred_db,
     pch = ".", col = rgb(1, 0, 0, 0.1), cex = 2,
     main = sprintf("Pred vs Actual dB — dB-aware\nR²=%.3f",
                     1 - sum((emission_db$pred_db - emission_db$actual_db)^2) /
...
pipeline_message(text = "Standard vs dB-aware comparison complete",
