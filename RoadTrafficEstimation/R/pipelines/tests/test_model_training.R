...
test_model_training <- function() {
  pipeline_message("Testing model training phase", level = 0, 
  ...
  pipeline_message("Test 1: XGBoost models file", level = 1, 
  ...
  if (file.exists(CFG$XGB_MODELS_WITH_RATIOS_FILEPATH)) {
    ...
    if (all(expected_base %in% names(models_list))) {
      pipeline_message(sprintf("All base models present (%d total models)", 
                               length(models_list)),
                       level = 1, progress = "end", process = "valid")
      tests_passed <- tests_passed + 1
    } else {
      missing <- setdiff(x = expected_base, y = names(models_list))
      pipeline_message(sprintf("Missing base models: %s", 
                               paste(missing, collapse = ", ")),
                       level = 1, progress = "end", process = "fail")
      tests_failed <- tests_failed + 1
    }
  } else {
    pipeline_message(sprintf("Models file not found: %s",
  ...
  pipeline_message("Test 2: Feature info file", level = 1, 
                   progress = "start", process = "search")
  
  if (file.exists(CFG$XGB_RATIO_FEATURE_INFO_FILEPATH)) {
    feature_info <- readRDS(file = CFG$XGB_RATIO_FEATURE_INFO_FILEPATH)
    
    if (!is.null(feature_info$road_feature_formula) && 
        !is.null(feature_info$all_periods)) {
      pipeline_message(sprintf("Feature info valid: %s periods", 
                               length(feature_info$all_periods)),
                       level = 1, progress = "end", process = "pass")
      tests_passed <- tests_passed + 1
    } else {
      pipeline_message("Feature info incomplete", level = 1, 
                       progress = "end", process = "fail")
      tests_failed <- tests_failed + 1
    }
  } else {
    pipeline_message(sprintf("Feature info file not found: %s", 
  ...
  pipeline_message("Test 3: Model quality metrics", level = 1, 
                   progress = "start", process = "search")
  ...
  if (exists("models_list") && length(models_list) > 0) {
    ...
    if (metrics_check) {
      pipeline_message("Model metrics valid", level = 1, 
                       progress = "end", process = "valid")
      tests_passed <- tests_passed + 1
    } else {
      pipeline_message("Some model metrics out of range", level = 1, 
                       progress = "end", process = "fail")
      tests_failed <- tests_failed + 1
    }
  } else {
    pipeline_message("Models not loaded, skipping metrics check", level = 1, 
                     progress = "end", process = "fail")
  }
  
  # Summary
  pipeline_message(sprintf("Model training tests: %d passed, %d failed", 
