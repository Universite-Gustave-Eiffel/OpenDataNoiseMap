...
#          VERSION
# ==============================================================================
# This stage performs the downloading of AVATAR data and the spatial matching of 
# AVATAR count points with OSM road segments. It is a minimal version of the 
# AVATAR data preparation stage, which focuses on downloading count point data 
# and matching it with OSM roads, without performing the full aggregation and 
# feature engineering steps.
# 
# Inputs:
#   - OSM_ROADS_CONNECTIVITY_FILEPATH: Pre-processed OSM road network with 
#     connectivity information
#   - AVATAR_API_TOKEN: API token for accessing AVATAR data
# Outputs:
#   - AVATAR_MERGED_WITH_OSM_FILEPATH: Spatial dataset of OSM roads matched with 
#     AVATAR count points, with count point attributes
#   - AVATAR_CSV_DATA_DIR: Directory containing downloaded AVATAR data chunks in 
#     CSV format
#   - AVATAR_RDS_DATA_FILEPATH: Combined AVATAR data in RDS format (after 
...
if (!exists(x = 'osm_full_network', inherits = FALSE)){
  ...
  if (sf::st_crs(x = osm_full_network) != CFG$TARGET_CRS){
...
if (file.exists(CFG$AVATAR_COUNT_POINTS_FILEPATH) && 
...
} else if (!file.exists(CFG$AVATAR_COUNT_POINTS_FILEPATH) && 
           RUN_CONTEXT == "local") {
  ...
  if (isTRUE(download_status)){
    ...
    level = 1, progress = "end", process = "valid")
           ...
           RUN_CONTEXT == "slurm") {
  pipeline_message(
    sprintf("Avatar data can't be downloaded from a Slurm job and the file %s is missing. Run the script locally", CFG$AVATAR_COUNT_POINTS_FILEPATH), 
    process = "stop")
}
...
count_points <- count_points %>% st_transform(crs = CFG$TARGET_CRS)
...
if (IS_TEST_MODE) {
  ...
  pipeline_message(sprintf("TEST MODE: Count points cropped %d → %d", 
                           n_before, nrow(count_points)),
...
sf::st_geometry(obj = pointsId_with_roads) <- 
    ...
    sf::st_distance(
      x          = count_points[
                      match(x     = pointsId_with_roads$id, 
                            table = count_points$id), ], 
      y          = osm_full_network[
                      match(x     = pointsId_with_roads$osm_id, 
...
# Project data into target CRS if needed
if (sf::st_crs(x = full_network_avatar_id) != CFG$TARGET_CRS){
...
pipeline_message(
  sprintf("OSM road network successfully merged with count point data and saved into file %s", 
          rel_path(CFG$AVATAR_MERGED_WITH_OSM_FILEPATH)), 
  level = 2, progress = "end", process = "valid")
...
if (nchar(CFG$AVATAR_API_TOKEN) == 0) {
  pipeline_message(
    paste0("AVATAR_API_TOKEN not set. Add it to ~/.Renviron:\n", 
...
if (CFG$FORCE_REDOWNLOAD_CHUNKS) {
  old_chunks <- list.files(path       = CFG$AVATAR_CSV_DATA_DIR, 
...
existing_files <- list.files(path       = CFG$AVATAR_CSV_DATA_DIR, 
                             pattern    = "avatar_data_chunk_.*\\.csv", 
                             full.names = TRUE)
...
for (chunk_id in 1:total_chunks) {
  chunk_file <- file.path(CFG$AVATAR_CSV_DATA_DIR, 
...
if (CFG$FORCE_REDOWNLOAD_CHUNKS && RUN_CONTEXT == "local") {
...
if (CFG$FORCE_REDOWNLOAD_MISSING_INVALID_CHUNKS && 
    length(chunks_to_redownload) > 0 && 
    RUN_CONTEXT == "local") {
  ...
  for (chunk_id in chunks_to_process) {
    ...
    # Output filename
    target_file <- file.path(CFG$AVATAR_CSV_DATA_DIR, 
                             paste0("avatar_data_chunk_", 
                                    sprintf("%03d", chunk_id), 
                                    ".csv"))
...
if (file.exists(CFG$AVATAR_RDS_DATA_FILEPATH)) {
  pipeline_message(sprintf("Loading cached Avatar data from %s", 
  ...
  pipeline_message(sprintf("Avatar data loaded from cache (%s rows)", 
                           fmt(nrow(avatar_data))),
    level = 2, progress = "end", process = "valid")
} else {

# List of files for all chunks
files <- list.files(path       = CFG$AVATAR_CSV_DATA_DIR, 
                    pattern    = "avatar_data_chunk_.*\\.csv", 
                    full.names = TRUE)
...
if (length(files) > 0) {
  ...
  # Read all CSV files with fread in batches to limit memory usage
  ...
  temp_combined_csv <- file.path(CFG$AVATAR_CSV_DATA_DIR,
  ...
  for (b in seq_len(n_batches)) {
    ...
    batch_list  <- lapply(batch_files, function(file) {
      tryCatch(
        expr  = data.table::fread(file             = file, 
                                  sep              = ";", 
    ...
    if (b %% 5 == 0) {
      ...
      if (!is.na(avail_gb) && avail_gb < 6) {
        pipeline_message(
          sprintf("Low memory during batch %d/%d: %.1f GB available", 
  ...
  # Read merged temporary CSV
  check_memory_available(
    operation_name = "Load merged Avatar temporary CSV",
    min_gb         = 3, 
    warn_gb        = 6)

  # Read the combined CSV file into a data frame
  ...
  pipeline_message(
    sprintf("Avatar data successfully combined (%s rows) and saved", 
            fmt(nrow(avatar_data))), 
...
rm(list = intersect(x = ls(), 
                    y = c("avatar_data", "valid_batches", "files")))
gc(verbose = FALSE)
