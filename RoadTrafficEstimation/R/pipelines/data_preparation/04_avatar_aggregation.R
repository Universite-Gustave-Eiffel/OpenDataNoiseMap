...
if (!exists(x= 'avatar_data', inherits = FALSE && 
  ...
  if (!is.na(avail_gb) && avail_gb < 6) {
    pipeline_message(
      sprintf("Low RAM detected (%.1f GB available) - switching to chunk-streaming mode", 
              avail_gb),
      process = "warning")
    ...
    pipeline_message(
      "Avatar data successfully loaded", 
...
if (isTRUE(use_chunk_streaming)) {
  ...
  if (length(files) == 0) {
    pipeline_message("No Avatar CSV chunk files found for chunk-streaming mode", 
                     process = "stop")
  }
  ...
  for (i in seq_along(files)) {
    ...
    if (i %% 25 == 0 || i == n_files) {
      pipeline_message(
        sprintf("Chunk-streaming progress: %d/%d files", i, n_files),
        process = "info")
    }
  }
  ...
  pipeline_message(
    "Hourly aggregation successfully completed in chunk-streaming mode", 
    level = 1, progress = "end", process = "valid")
} else {
  pipeline_message(
    "Conversion of Avatar data into a data table and creation of period and hour columns", 
...
pipeline_message("Calculating weekday/weekend hourly traffic flow metrics", 
                 level = 1, progress = "start", process = "calc")
...
pipeline_message(
  sprintf("Weekday/weekend hourly metrics: %s wd rows, %s we rows", 
...
pipeline_message(
  sprintf("Traffic flow relative metrics sucessfully calculated and saved into file %s", 
          rel_path(CFG$AVATAR_AGGREGATED_FILEPATH)), 
  level = 1, progress = "end", process = "valid")
...
pipeline_message(
  sprintf("Hourly aggregated data successfully plotted and saved into file %s", 
          paste(rel_path(CFG$FIGS_DIR), CFG$FIG_HOURLY_TRAFFIC_FILENAME, 
...
pipeline_message(
  sprintf("Hourly speed and truck percentage successfully plotted and saved into file %s", 
          paste(rel_path(CFG$FIGS_DIR), 
                CFG$FIG_SPEED_AND_TRUCK_PERCENTAGE, 
                sep = .Platform$file.sep)), 
  level = 1, progress = "end", process = "valid")
...
pipeline_message(
  sprintf("Period comparison (D/E/N) successfully plotted and saved into file %s", 
          paste(rel_path(CFG$FIGS_DIR), 
                CFG$FIG_TRAFFIC_PERIOD_COMPARISONS, 
                sep = .Platform$file.sep)), 
  level = 1, progress = "end", process = "valid")
...
pipeline_message(
  sprintf("Traffic flow distribution and data quality successfully plotted and saved into file %s", 
          paste(rel_path(CFG$FIGS_DIR), 
                CFG$FIG_TRAFFIC_FLOW_DISTRIB_AND_DATA_QUALITY, 
                sep = .Platform$file.sep)), 
  level = 1, progress = "end", process = "valid")
