...
plot_hourly_traffic_profile <- function(traffic_hourly_patterns,
  ...
  p <- ggplot(data = traffic_hourly_patterns, mapping = aes(x = hour)) +
    geom_ribbon(mapping = aes(ymin = q25_flow, 
                              ymax = q75_flow), 
                fill    = "steelblue", 
                alpha   = 0.3) + 
    ...
    scale_x_continuous(breaks = seq(from = 0, to = 23, by = 2), 
                       labels = paste0(seq(from = 0, to = 23, by = 2), "h")) + 
...
plot_flow_distribution_and_quality <- function(aggregated_traffic_data,
  ...
  p_flow <- ggplot(data    = flow_data, 
                   mapping = aes(x = aggregate_flow)) + 
    geom_histogram(bins  = 50, 
                   fill  = "steelblue", 
                   color = "white", 
                   alpha = 0.8) +
    geom_vline(xintercept = median(x     = flow_data$aggregate_flow, 
                                   na.rm = TRUE), 
               color      = "red", 
               linetype   = "dashed", 
               linewidth  = 1) + 
  ...
  p_quality <- ggplot(data    = quality_data, 
