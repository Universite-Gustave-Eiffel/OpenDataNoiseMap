...
rm(models_list, feature_info, osm_pemb_dt)
gc(verbose = FALSE)
...
check_memory_available(
  operation_name = sprintf("Pivot to long format (%s roads × %d periods)",
                           fmt(nrow(predictions_wide)),
                           length(unique(gsub("^(flow|truck_pct|speed)_", "",
                                              grep("^flow_", names(predictions_wide), value = TRUE))))),
  min_gb = 2, warn_gb = 4)
